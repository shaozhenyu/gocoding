...
func findAllConcatenatedWordsInADict(words []string) []string {
	...
	for i := 0; i < len(words); i++ {
		...
		if dtree.Search(dtree, words[i], 0) {
...
func (this *WordDictionary) print() {
	fmt.Println(this.exist, this.isEnd)
	for i := 0; i < 26; i++ {
		if this.exist[i] {
			fmt.Println(i, this.isEnd)
			this.child[i].print()
		}
	}
}
...
func (this *WordDictionary) AddWord(word string) {
	...
	this.child[v].AddWord(word[1:])
}


func (this *WordDictionary) Search(root WordDictionary, word string, count int) bool {
	// fmt.Println(len(word), word, count, this.isEnd)
	if len(word) == 0 {
		return count >= 1 && this.isEnd
	}
	if this.isEnd && root.Search(root, word, count+1) {
		return true
	}
	v := word[0] - 'a'
	if this.exist[v] && this.child[v].Search(root, word[1:], count) {
		return true
	}
	return false
}
