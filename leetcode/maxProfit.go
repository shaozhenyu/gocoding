...
func maxProfit1(prices []int) int {
	...
	return do(prices, win, buy, sell, cooldown)
}

func do(prices []int, win int, buy, sell int, cooldown bool) int {
	if len(prices) == 0 {
		return win
	}

	if cooldown == true {
		return do(prices[1:], win, buy, sell, false)
	}

	if buy != -1 && prices[0] < buy {
		return do(prices[1:], win, buy, sell, cooldown)
	}

	var b, c int
	//a = do(prices[1:], win, buy, sell, cooldown)

	if buy == -1 {
		b = do(prices[1:], win, prices[0], sell, cooldown)
	}
	...
	if buy != -1 {
		sell = prices[0]
		win += sell - buy
		fmt.Println("selll - buy", sell, buy)
		buy = -1
		c = do(prices[1:], win, buy, sell, true)
	}
