...
func (n *BNode) delete(key int, idx int) *BNode {
	...
	if n.Child[idx] != nil {
		...
		n.Num--
		for n.Num < (M-1)/2 && n.Parent != nil && n.Parent.Num > 0 {
			var ok bool
			ok, n = n.restore()
			if !ok {
				n = n.mergeNode()
	...
	fmt.Println("aa: ", n.Key, n.Num, n.Parent)
...
func (n *BNode) restore() (bool, *BNode) {
	...
	if i < parent.Num {
		b := parent.Child[i+1]
		if b.Num > (M-1)/2 {
			fmt.Println("restore right: ", n.Key)
			n.Num++
			n.Key[n.Num] = parent.Key[i+1]
			n.Child[n.Num] = b.Child[0]
			parent.Key[i+1] = b.Key[1]
			for j := 1; j < b.Num; j++ {
				b.Key[j] = b.Key[j+1]
				b.Child[j-1] = b.Child[j]
			}
			b.Child[b.Num-1] = b.Child[b.Num]
			b.Num--
			return true, parent
		}
	}
...
func (n *BNode) mergeNode() *BNode {
	...
	// 存在左兄弟节点
	if i > 0 {
		fmt.Println("merge left:", n.Key, n.Num, i)
		b := parent.Child[i-1]
		b.Num++
		...
		for j := i; j < parent.Num; j++ {
			parent.Key[j] = parent.Key[j+1]
			parent.Child[j] = parent.Child[j+1]
		}
		parent.Num--
		parent.Child[i-1] = b
		fmt.Println(parent.Num)
